...
- Activity uses a fixed, linear signed-rate color scale: −1 blue, 0 gray, +1 coral. Dot size encodes magnitude. Pale rings encode absolute change from the previous received sample, with fixed 8× gain capped at 1; they do not invent spikes. A new run or controller source starts without a change ring. The canvas checks for observations every animation frame instead of throttling to 15 fps. The physics client supplies observations at up to 25 Hz; unchanged states remain unchanged.
...
- Drawing shows the learned feedback controller's actual rate state; idle wing flapping shows the learned circle planner's state. Rate values are engineering approximations, not recorded spikes.
